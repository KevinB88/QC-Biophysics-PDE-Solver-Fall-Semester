...
# Only keeping track of the past and current states the AL and DL, for memory optimization purposes
def solve_short(rings, rays, r, d, t, q, p, a, b, v, tube_placements, output_specification):
    ...
    if q:
        info_dict = pr.pr_quantities(rings, rays, r, d, t, d_radius, d_theta, d_time, 2, phi_center, a, b, v, tube_placements)
...
# **** - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    for k in range(time_steps):
        ...
        for m in range(rings):
            angle_index = 0
            if m == rings - 2:
                mass_loss_tab[k] = calc_loss_mass_j_r_r(diffusive_layer, 0, d_radius, d_theta, rings, rays)

                if k % 2 == 0:
                    mass_loss_tab_op[0] = calc_loss_mass_j_r_r(diffusive_layer, 0, d_radius, d_theta, rings, rays)
                else:
                    mass_loss_tab_op[1] = calc_loss_mass_j_r_r(diffusive_layer, 0, d_radius, d_theta, rings, rays)

                if k > 0:
                    print(f'past: {mass_loss_tab[k-1]}, current: {mass_loss_tab[k]}')
                    print(f'past: {mass_loss_tab_op[0]}, current: {mass_loss_tab_op[1]}')
                if k > 0 and k % 2 != 0 and global_max_flag:
                    if mass_loss_tab[k-1] > mass_loss_tab[k]:
                        ...
                        global_max_flag = False
                        clk.sleep(5)
                    if mass_loss_tab_op[0] > mass_loss_tab_op[1]:
                        print(f'glb peak time attained: {glb_peak_time}')
                        clk.sleep(5)
            ...
            for n in range(rays):
                if m == rings - 1:
                    ...
                    # diffusive_layer[0][m][n] = diffusive_layer[1][m][n]
                    if n == tube_placements[angle_index]:
                        ...
                        # advective_layer[0][m][n] = advective_layer[1][m][n]
        ...
        mass_tab[k] = calc_mass(diffusive_layer, advective_layer, 0, d_radius, d_theta, phi_center, rings, rays, tube_placements)
...
# Keeping track of the entire history of the AL and DL, more costly on memory usage during longer simulations
def solve_history(rings, rays, r, d, t, q, p, a, b, v, tube_placements, output_specification):
    ...
    central_phi_tab = np.zeros([time_steps+1])
    mass_tab = np.zeros([time_steps])
    mass_loss_tab = np.zeros([time_steps])
    mass_loss_tab_op = np.zeros([2])
...
# **** - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    for k in range(time_steps):
        ...
        for m in range(rings):
            ...
            for n in range(rays):
                if m == rings - 1:
                    ...
                    if n == tube_placements[angle_index]:
                        # Update the associated tube within the dictionary
                        advective_layer[k][m][n] = u_tube(advective_layer, diffusive_layer, k+1, m, n, a, b, v, d_time, d_radius, d_theta)
                        # advective_layer[0][m][n] = advective_layer[1][m][n]
                        if angle_index < len(tube_placements)-1:
                            angle_index = angle_index + 1
                if m == rings - 2:
        ...
        # mass_tab[k] = calc_mass(diffusive_layer, advective_layer, k, d_radius, d_theta, phi_center, rings, rays, rho_mass_tab, phi_mass_tab, tube_placements)
        mass_tab[k] = calc_mass(diffusive_layer, advective_layer, k, d_radius, d_theta, phi_center, rings, rays, tube_placements)
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    # New implement feature added on 9/13/24 at 2:44 PM
    # 9/20/24: 2:21 PM A dictionary of the values should be outputted from the list, the appropriate operation should be committed to each within the Main function
    output_dictionary = {
        'mean-flow-passage-time': m_f_p_t,
        'diffusive_layer': diffusive_layer,
        'central_patch': central_phi_tab,
