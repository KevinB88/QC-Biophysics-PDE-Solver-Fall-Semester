...
# Make changes to the plot, 9/18/24 : 11:37 PM, changes have been made for testing purposes

def plt_phi_rad_dual(container, radius, angle, save_png, filepath, time, plot_show, time_pos):

    final_state = container[time_pos]

    radial_densities = np.zeros([len(container[0])])
    ...
    plt.title(f'Phi versus radius at angle: {angle} at time t={time}.')
    plt.plot(x, y, 'black')
    plt.xlabel('Radius')
    plt.ylabel('Density')
...
def plt_phi_rad(container, radius, angle, save_png, filepath, time, plot_show, d_time, time_point):
    ...
    plt.title(f'Phi versus radius at angle: {angle} at time t={time_stamp}, equates to time-step: {time_stamp}')
    plt.plot(x, y, 'black')
    plt.xlabel('Radius')
    plt.ylabel('Density')
